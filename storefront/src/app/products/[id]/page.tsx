...
export default function ProductDetailPage() {
  ...
  const buildSelectedOptions = () => {
    ...
    const opts: Array<{ optionId: number; valueId: number; value: string; priceAdjustment: number }> = [];
    for (const opt of product.options) {
      const valId = selected[opt.slug];
      if (!valId) continue;
      const val = opt.values.find((v) => String(v.id) === valId);
      if (!val) continue;
      opts.push({ optionId: opt.id, valueId: val.id, value: val.value, priceAdjustment: val.priceAdjustment });
    }
  ...
  const handleBuyNow = () => {
    if (!product) return;
    addItem(product.id, buildSelectedOptions());
    trackAddToCart({
      id: product.id,
